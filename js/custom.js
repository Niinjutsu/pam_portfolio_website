    ...
    document.querySelectorAll(".share-btn").forEach((btn) => {
      btn.addEventListener("click", function (e) {
        ...
        const projectURL = document.querySelector(
          `.project-direct-link[data-project="${projectId}"]`
        ).textContent;
        const projectTitle = this.closest(
...
function initForms() {
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", function (event) {
      ...
      const mailtoLink = `mailto:clayprince14353@gmail.com?subject=${encodeURIComponent(
        name
      )} - ${encodeURIComponent(
        company
...
function initShareButtons() {
  ...
  shareButtons.forEach((button) => {
    button.addEventListener("click", function (e) {
      ...
      const projectUrl = document.querySelector(
        `.project-direct-link[data-project="${project}"]`
      ).textContent;
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      const copyButton = document.querySelector(
        `.share-btn.copy[data-project="${projectId}"]`
      );
      if (copyButton) {
        ...
        const copiedMessage =
          copyButton.closest(".share-container").nextElementSibling;
        if (
          copiedMessage &&
          copiedMessage.classList.contains("project-link-copied")
        ) {
